...
namespace GGC.Scheduler
{
    public class SQLHelper
    {
        ...
        {
            return ConfigurationManager.AppSettings["DevpGridConnectivity"].ToString();
        }

       
        /// <summary>
        /// Verifies whether the called connection is not down and thus checks its state
        /// </summary>
        /// <returns></returns>
        public static bool GetConnectionState()
        {
            MySqlConnection oraConnPMS = new MySqlConnection();
            MySqlConnection oraConnOnlineSell = new MySqlConnection();
            bool blnConnState = false;
            try
            {
                if (oraConnPMS.State == ConnectionState.Open)
                {
                    oraConnPMS.Close();
                }

                if (oraConnOnlineSell.State == ConnectionState.Open)
                {
                    oraConnOnlineSell.Close();
                }

                if ((oraConnPMS.State != ConnectionState.Open) && (oraConnOnlineSell.State != ConnectionState.Open))
                {
                    oraConnPMS.ConnectionString = ConfigurationManager.AppSettings["PMSConn"].ToString();
                    oraConnPMS.Open();


                    oraConnOnlineSell.ConnectionString = ConfigurationManager.AppSettings["DevpGroupPay"].ToString();
                    oraConnOnlineSell.Open();
                }
            ...
            {
                //If database is down then return 'false' so as to redirect the user to Under Maintenance Page.
        ...
        // added by snehal on 25 aug 2011 
        // to be used only by admin module of onlineselling 
        public static int ExecuteNonQuery(string module, MySqlConnection connection, CommandType commandType, string commandText, params MySqlParameter[] commandParameters)
        {
            int retval;
            try
            {
        ...
        /// <param name="commandParameters">An array of SqlParamters used to execute the command</param>
        /// <returns>A dataset containing the resultset generated by the command</returns>
        public static DataSet ExecuteDataset(string connectionString, CommandType commandType, string commandText, string SrcTable)
        {
            //pass through the call providing null for the set of MySqlParameters
            return ExecuteDataset(connectionString, commandType, commandText, SrcTable, (MySqlParameter[])null);
        }
        public static DataSet ExecuteDataset(string connectionString, CommandType commandType, string commandText, string SrcTable, params MySqlParameter[] commandParameters)
        {
            //create & open a MySqlConnection, and dispose of it after we are done.
            ...
            try
            {
                ...
                //call the overload that takes a connection in place of the connection string
                return ExecuteDataset(cn, commandType, commandText, SrcTable, commandParameters);
            }
            finally
            {
                cn.Dispose();
        ...
        public static DataSet ExecuteDatasetReturnProviderSpecific(MySqlConnection connection, CommandType commandType, string commandText, params MySqlParameter[] commandParameters)
        {
            if (connection == null) throw new ArgumentNullException("connection");
        ...
        public static bool UpdateDataset(string ssql, ref DataSet dsDataset, string strTableName)
        {
            MySqlDataAdapter mDataAdapter = new MySqlDataAdapter(ssql, GetConnection());
            MySqlCommandBuilder mDB = new MySqlCommandBuilder(mDataAdapter);
            //mDataAdapter.InsertCommand = mDB.GetInsertCommand();
            //mDataAdapter.UpdateCommand = mDB.GetUpdateCommand();
            //mDataAdapter.DeleteCommand = mDB.GetDeleteCommand();
            try
            {
                mDataAdapter.Update(dsDataset, strTableName);
        ...
        public static bool UpdateMasterChildDataset(string[] ssql, ref DataSet dsDataset, string[] strTableName, string AutoGeneratedColumn)
        {

            MySqlConnection con = new MySqlConnection(GetConnection());
            DataSet objTempDataSet = new DataSet();

            FillDataset(con, CommandType.Text, ssql[0].ToString(), objTempDataSet, new string[] { strTableName[0] });
            FillDataset(con, CommandType.Text, ssql[1].ToString(), objTempDataSet, new string[] { strTableName[1] });

            MySqlTransaction objOracleTrans;
            ...
            {
                con.Open();
                objOracleTrans = con.BeginTransaction();
                com.Transaction = objOracleTrans;

                //process parent table

                foreach (DataRow pRow in dsDataset.Tables[0].Rows)
                {
                    DataRow pnewRow = objTempDataSet.Tables[0].NewRow();
                    foreach (DataColumn pDc in dsDataset.Tables[0].Columns)
                    {
                        pnewRow[pDc.ColumnName] = pRow[pDc.ColumnName];
                    }
                    objTempDataSet.Tables[0].Rows.Add(pnewRow);
                }

                MySqlDataAdapter mDataAdapter = new MySqlDataAdapter();
                com.CommandText = ssql[0].ToString();
                mDataAdapter.SelectCommand = com;
                MySqlCommandBuilder mDB = new MySqlCommandBuilder(mDataAdapter);
                mDataAdapter.Update(objTempDataSet.Tables[0]);

                //get Max id of auto generated column

                string strQuery = "select max(" + AutoGeneratedColumn + ") from " + dsDataset.Tables[0].TableName;
                com.CommandText = strQuery;
                object autoObj = com.ExecuteScalar();

                //////process child table

                foreach (DataRow cRow in dsDataset.Tables[1].Rows)
                {
                    DataRow cnewRow = objTempDataSet.Tables[1].NewRow();
                    for (int colCount = 0; colCount < dsDataset.Tables[1].Columns.Count; colCount++)
                    {
                        if (colCount == 0)
                        {
                            cnewRow[colCount] = autoObj;
                        }
                        else
                        {
                            cnewRow[colCount] = cRow[colCount];
                        }
                    }
                    objTempDataSet.Tables[1].Rows.Add(cnewRow);
                }

                com.CommandText = ssql[1].ToString();
                mDataAdapter.SelectCommand = com;
                mDB = new MySqlCommandBuilder(mDataAdapter);
                int retVal = mDataAdapter.Update(objTempDataSet.Tables[1]);
                objOracleTrans.Commit();
                return true;
        ...
        public static bool ValidateQueryResult(string strSQL)
        {
            MySqlConnection oOracleConn = null;
            MySqlCommand oOracleComm = null;
            bool userExists = true;
            try
            {
                oOracleConn = new MySqlConnection(GetConnection());
                oOracleConn.Open();
                oOracleComm = new MySqlCommand(strSQL, oOracleConn);
                MySqlDataReader oOraclerdr = oOracleComm.ExecuteReader();
                if (!oOraclerdr.Read())
                    userExists = false;
            }
            catch (Exception ex)
        ...
        public static System.Data.DataSet ExecuteDataset(string strSpName, Hashtable ht)
        {
            object[] parameters;
            if (ht == null)
            {
                Hashtable ht1 = new Hashtable(0);
                ht1.Add("RSET", null); //This need to be declared as RSET in SP
                parameters = SQLHelperParameterCache.SetParameter(GetConnection(), strSpName, ht1);
            }
            else
            {
                parameters = SQLHelperParameterCache.SetParameter(GetConnection(), strSpName, ht);
            }

            DataSet ds = SQLHelper.ExecuteDataset(GetConnection(), strSpName, parameters);
            return ds;
        }
    }
    ...
    public sealed class MySqlTransactions
    {
