...
{
    public partial class CommFeeList : System.Web.UI.Page
    {
        protected static readonly ILog log = LogManager.GetLogger(typeof(CommFeeList));
            ...
            try
            {
                ...
                switch (mySqlConnection.State)
                {
                    ...
                    case System.Data.ConnectionState.Open:
                        ...
                        strQuery = "SELECT distinct a.*,b.* FROM applicantdetails a, proposalapproval b  WHERE a.APPLICATION_NO=b.APPLICATION_NO and b.roleid='53' and isProposalpprove='Y' and WF_STATUS_CD_C=14";
        ...
        protected void GVApplications_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            if (e.CommandName == "UploadPPA")
            {
                Response.Redirect("~/UI/Emp/UploadPPA.aspx?application=" + applicationId, false);
            }
            if (e.CommandName == "IssueDemandNote")
            {
                ...
                //    strQuery = "update applicantdetails set  CommittmentFees=, IsCommittmentFeesPaid='Y' where APPLICATION_NO='" + applicationId + "'";
                //    cmd = new MySqlCommand(strQuery, mySqlConnection);
                //    cmd.ExecuteNonQuery();
        ...
        protected void sendEmail(string strAppID)
        {
            ...
            try
            {
                ...
                strQuery = "update applicantdetails set WF_STATUS_CD_C=15, CommittmentFees='" + projectCapacityAmount + "',app_status='DEMAND NOTE ISSUED.',IssueLetter='Y'  where APPLICATION_NO='" + strAppID + "'";
                log.Error("Update query - "+strQuery);
                cmd = new MySqlCommand(strQuery, mySqlConnection);
                cmd.ExecuteNonQuery();
                
                objSaveToMEDA.saveApp(Session["PROJID"].ToString(), "15");
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                try
                {
                    if (e.Row.RowType == DataControlRowType.DataRow)
                    {
                        ...
                        string appStatus = ds.Tables[0].Rows[rowno]["app_status"].ToString();
                        //PPA letter uploaded.
                        ...
                        if (wfStatus == 14)
                        {
                            ...
                            {
                                if(appStatus.ToUpper() == "PPA letter uploaded.".ToUpper())
                                {
