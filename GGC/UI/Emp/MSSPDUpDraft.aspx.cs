...
{
    public partial class MSSPDUpDraft : System.Web.UI.Page
    {
        protected static readonly ILog log = LogManager.GetLogger(typeof(MSSPDUpDraft));
        string strAppID;
        ...
        {
            string folderPath = Server.MapPath("~/Files/MSKVY/SPD/" + strAppID + "/");
            ...
            string newFileName = "";
                ...
                if (FUDoc.HasFile)
                {
                    try
                    {
                        ...
                        string strQuery = "Update mskvy_applicantdetails_spd set WF_STATUS_CD_C=13, APP_STATUS_DT=CURDATE() , app_status='Proposal name change document Uploaded.' where APPLICATION_NO='" + strAppID + "'";
                        ...
                        strQuery = "insert into mskvy_upload_doc_spd (Application_No, FileType, FieName, CreateDT, CreateBy) " +
                                " values('" + strAppID + "',12,'" + newFileName + "',now(),'" + Session["SAPID"].ToString() + "')";
                        SQLHelper.ExecuteNonQuery(conString, CommandType.Text, strQuery);

                        objSaveToMEDA.saveMSKVYApp(Session["PROJID"].ToString(), "13");
           
                        lblResult.Text = "Proposal name change draft GC document Uploaded Successfully!!";

                        lblResult.ForeColor = System.Drawing.Color.Green;

                        strQuery = "insert into proposalapproval_SPD(APPLICATION_NO , roleid , createDT ,createBy) values ('" + strAppID + "','51', now() , '" + Session["SAPID"].ToString() + "')";
                        SQLHelper.ExecuteNonQuery(conString, CommandType.Text, strQuery);

                        #region Application Status tracking date
                        strQuery = "insert into APPLICANT_STATUS_TRACKING " +
                                " values('" + strAppID + "','Proposal name change document uploaded.',now(),'" + Session["SAPID"].ToString() + "')";
