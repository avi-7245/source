...
{
    public partial class Feasibility : System.Web.UI.Page
    {
        ...
        protected void btnApproved_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                strQuery = "select * from proposalapproval where APPLICATION_NO='" + strAppID + "'";
                        ...
                        MySqlDataAdapter da = new MySqlDataAdapter(cmd);
                        da.Fill(dsResultStatus);
                        log.Error("dsResultStatus.Tables[0].Rows.Count " + dsResultStatus.Tables[0].Rows.Count);
                        if (dsResultStatus.Tables[0].Rows.Count > 0)
                        {
                            strQuery = "update proposalapproval set  isAppr_Rej_Ret='Y',roleid ='51',remark='" + txtRemark.Text + "', createDT=now() ,  createBy='" + Session["SAPID"].ToString() + "' where APPLICATION_NO ='" + strAppID + "'";
                        ...
                        log.Error("proposal approval " + strQuery);
                cmd = new MySqlCommand(strQuery, mySqlConnection);
                cmd.ExecuteNonQuery();
                ...
                strQuery = "Update APPLICANTDETAILS set WF_STATUS_CD_C=12, app_status='PROPOSAL FEASIBILITY VERIFIED.' ,APP_STATUS_DT=CURDATE() where APPLICATION_NO='" + strAppID + "'";
                cmd = new MySqlCommand(strQuery, mySqlConnection);
                cmd.ExecuteNonQuery();
                ...
                lblProjLoc.Text = "";
                txtRemark.Text = "";
                ScriptManager.RegisterStartupScript(this, this.GetType(), "alert", "alert('Fesasibility verified.');", true);
            }
            catch (Exception exception)
            {
                string ErrorMessage = "Method Name: " + MethodBase.GetCurrentMethod().Name + " | Description: " + exception.Message + " " + exception.InnerException;
        ...
        protected void btnReject_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                cmd = new MySqlCommand(strQuery, mySqlConnection);
                cmd.ExecuteNonQuery();
                strQuery = "Update APPLICANTDETAILS set WF_STATUS_CD_C=11,  app_status='Proposal feasibility  rejected.' ,APP_STATUS_DT=CURDATE() where APPLICATION_NO='" + strAppID + "'";
                cmd = new MySqlCommand(strQuery, mySqlConnection);
                cmd.ExecuteNonQuery();
