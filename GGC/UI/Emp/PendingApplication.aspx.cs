...
namespace GGC.UI.Emp
{
    ...
    {
        protected static readonly ILog log = LogManager.GetLogger(typeof(PendingApplication));
            ...
            try
            {
                ...
                switch (mySqlConnection.State)
                {
                    ...
                    case System.Data.ConnectionState.Open:
                        ...
                        if (rollid <= 10)
                        {
                            //strQuery = "select * ,date_format(MEDA_REC_LETTER_DT,'%Y-%m-%d') MEDADateF,date_format(APP_STATUS_DT,'%Y-%m-%d') APP_STATUS_DT,date_format(SCHEDULED_COMM_DATE,'%Y-%m-%d') SCHEDULED_COMM_DATE_F,datediff(curdate(),paymentdate) as days  from APPLICANTDETAILS where MEDALettername is null or IsPaymentDone is null";
                            strQuery = "select * ,date_format(MEDA_REC_LETTER_DT,'%Y-%m-%d') MEDADateF,date_format(APP_STATUS_DT,'%Y-%m-%d') APP_STATUS_DT,date_format(SCHEDULED_COMM_DATE,'%Y-%m-%d') SCHEDULED_COMM_DATE_F,datediff(curdate(),paymentdate) as days ,b.ORGANIZATION_NAME,concat(a.PROJECT_LOC,' ', a.PROJECT_TALUKA,' ', a.PROJECT_DISTRICT) as Location  from APPLICANTDETAILS a,applicant_reg_det b where a.USER_NAME=b.USER_NAME and WF_STATUS_CD_C<=3 order by a.CREATED_DT desc ";
                        }
                        else
                        {
                            strQuery = "select * ,date_format(MEDA_REC_LETTER_DT,'%Y-%m-%d') MEDADateF,date_format(APP_STATUS_DT,'%Y-%m-%d') APP_STATUS_DT,date_format(SCHEDULED_COMM_DATE,'%Y-%m-%d') SCHEDULED_COMM_DATE_F,datediff(curdate(),paymentdate) as days ,b.ORGANIZATION_NAME,concat(a.PROJECT_LOC,' ', a.PROJECT_TALUKA,' ', a.PROJECT_DISTRICT) as Location  from APPLICANTDETAILS a,applicant_reg_det b where a.USER_NAME=b.USER_NAME and WF_STATUS_CD_C<=3 and ZONE='" + Session["EmpZone"].ToString() + "' order by a.CREATED_DT desc ";
