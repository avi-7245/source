...
{
    public partial class MSKVYUploadIssueLetter : System.Web.UI.Page
    {
        ...
        {
            string folderPath = Server.MapPath("~/Files/IssueLetter/");
                    ...
                    try
                    {
                        ...
                        switch (mySqlConnection.State)
                        {
                            ...
                            case System.Data.ConnectionState.Open:
                                string strQuery = "Update MSKVY_applicantdetails  set WF_STATUS_CD_C=19, IssueLetter='" + newFileName + "' , IssueLetterDt=CURDATE() , app_status='GRID CONNECTIVITY LETTER ISSUED.' where APPLICATION_NO='" + strAppID + "'";
                                MySqlCommand cmd = new MySqlCommand(strQuery, mySqlConnection);
                                cmd.ExecuteNonQuery();
                                lblResult.Text = "Grid Connectivity Issued Letter Uploaded Successfully!!";
                                ...
                                strQuery = "insert into APPLICANT_STATUS_TRACKING " +
                                        " values('" + strAppID + "','Grid Connectivity Letter Issued Uploaded.',now(),'" + Session["SAPID"].ToString() + "')";
                                cmd = new MySqlCommand(strQuery, mySqlConnection);
                                cmd.ExecuteNonQuery();

                                strQuery = "select a.* from MSKVY_applicantdetails  a where a.APPLICATION_NO='" + strAppID + "'";
                                ...
                                strBody = "M/s. " + dsResult.Tables[0].Rows[0]["PROMOTOR_NAME"].ToString() + " , <br/>" +
                                    "Your Grid connectivity Letter is issued for application no " + strAppID + ". Kindly download attachement. <br/>";
