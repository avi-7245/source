...
{
    public partial class MSKVYFeasibilityConsent : System.Web.UI.Page
    {
        protected static readonly ILog log = LogManager.GetLogger(typeof(MSKVYFeasibilityConsent));
        ...
        void fillData()
        {
            ...
            try
            {
                ...
                strQuery = "select * from LFSFeasibilityStatus where APPLICATION_NO='" + strAppID + "' and FeasibilityApprStatus='R' order by StatusDt desc";
        ...
        protected void btnApproved_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                cmd = new MySqlCommand(strQuery, mySqlConnection);
                cmd.ExecuteNonQuery();
                lblResult.Text = "Approved Successfully";
                lblResult.ForeColor = System.Drawing.Color.Green;

                //strQuery = "insert into proposalapproval(APPLICATION_NO , roleid , createDT ,createBy) values ('" + strAppID + "','" + Session["EmpRole"].ToString() + "', now() ,'" + Session["SAPID"].ToString() + "')";
                ...
                strQuery = "update proposalapproval set isAppr_Rej_Ret='Y',Aprove_Reject_Return_by='" + Session["SAPID"].ToString() + "',roleid='51',remark='"+txtRemark.Text+"' where APPLICATION_NO= '" + strAppID + "'";
                ...
                strQuery = "Update MSKVY_applicantdetails  set WF_STATUS_CD_C=13, app_status='PROPOSAL FEASIBILITY CONSENT VERIFIED.' ,APP_STATUS_DT=CURDATE() where APPLICATION_NO='" + strAppID + "'";
                cmd = new MySqlCommand(strQuery, mySqlConnection);
                cmd.ExecuteNonQuery();
                ...
                strQuery = "insert into APPLICANT_STATUS_TRACKING " +
                        " values('" + strAppID + "','Proposal feasibility consent approved.',now(),'" + Session["SAPID"].ToString() + "')";
                cmd = new MySqlCommand(strQuery, mySqlConnection);
                cmd.ExecuteNonQuery();

                lblResult.Text = "Approved/Verified Successfully.";
                #endregion
        ...
        protected void btnReject_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                strQuery = "insert into LFSFeasibilityStatus(APPLICATION_NO,FeasibilityApprStatus,interconnectionarrangementremark,Remarks,StatusDt) values ('" + strAppID + "','N','" + txtIAR.Text + "', '" + txtRemark.Text + "',now())";


                cmd = new MySqlCommand(strQuery, mySqlConnection);
                cmd.ExecuteNonQuery();

                strQuery = "Update MSKVY_applicantdetails  set WF_STATUS_CD_C=12, app_status='Proposal feasibility consent rejected.' ,APP_STATUS_DT=CURDATE() where APPLICATION_NO='" + strAppID + "'";
                cmd = new MySqlCommand(strQuery, mySqlConnection);
                cmd.ExecuteNonQuery();
                ...
                #region Application Status tracking date
                strQuery = "insert into APPLICANT_STATUS_TRACKING " +
                        " values('" + strAppID + "','Proposal feasibility consent rejected.',now(),'" + Session["SAPID"].ToString() + "')";
                cmd = new MySqlCommand(strQuery, mySqlConnection);
                cmd.ExecuteNonQuery();
                #endregion

                //sendEmail();
                lblResult.Text = "Application Rejected.";
                lblApplcationNo.Text = "";
                lblNatOfApp.Text = "";
                lblProjCap.Text = "";
                lblProjectType.Text = "";
