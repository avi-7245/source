...
namespace GGC.UI.MSKVY
{
    public partial class PayRegConfirm : System.Web.UI.Page
    {
        ...
        protected void btnPay_Click(object sender, EventArgs e)
        {
            ...
            string strRU = ConfigurationManager.AppSettings["MSKVYRU"].ToString();
            string strCon = ConfigurationManager.AppSettings["ConventionalFees"].ToString();
            string strNonCon = ConfigurationManager.AppSettings["SPVFees"].ToString();
            ...
            try
            {
                ...
                switch (mySqlConnection.State)
                {
                    ...
                    case System.Data.ConnectionState.Open:
                        ...
                        MySqlCommand cmd = new MySqlCommand("select count(1) cnt  from BillDesk_TXN where ApplicationNo='" + applicationId + "' and AuthStatus is not null and typeofpay='MSKVYRegistration'", mySqlConnection);
                        ...
                        if (cnt < 1)
                        {
                            ...
                            string strQuery = "insert into BillDesk_TXN(ApplicationNo,MEDAProjectID, MerchantID, TxnAmount,ORIGINALSTATUS,  CreatedDt,typeofpay) " +
                                "values('" + applicationId + "','" + strProjectID + "','" + strMerchantCode + "','" + strPayAmt + "','" + "1',now(),'MSKVYRegistration')";
                            log.Error("3 : "+strQuery);
