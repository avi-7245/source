...
{
    public partial class PayCommConfirm : System.Web.UI.Page
    {
        protected static readonly ILog log = LogManager.GetLogger(typeof(PayCommConfirm));
        protected void Page_Load(object sender, EventArgs e)
        {
            ViewState["payApplicationId"] = Request.QueryString["appID"].ToString();
            ViewState["OrgName"] = Request.QueryString["OrgName"].ToString();

            lblOrgName.Text = ViewState["OrgName"].ToString();
        ...
        protected void btnPay_Click(object sender, EventArgs e)
        {
            ...
            string strRU = ConfigurationManager.AppSettings["MSKVYCOMMRU"].ToString();
            ...
            try
            {
                ...
                switch (mySqlConnection.State)
                {
                    ...
                    case System.Data.ConnectionState.Open:
                        ...
                        MySqlCommand cmd = new MySqlCommand("select count(1) cnt  from BillDesk_TXN where ApplicationNo='" + applicationId + "' and AuthStatus is not null and typeofpay='MSKVYCommittment'", mySqlConnection);
                        ...
                        if (cnt < 1)
                        {
                            ...
                            string strQuery = "insert into BillDesk_TXN(ApplicationNo,MEDAProjectID, MerchantID, TxnAmount,ORIGINALSTATUS,  CreatedDt,typeofpay) " +
                                "values('" + applicationId + "','" + strProjectID + "','" + strMerchantCode + "','" + strPayAmt + "','" + "1',now(),'MSKVYCommittment')";
...
// Change on 31.05.23 for oragnization name having  brackets    String data = strMerchantCode + "|" + strSrno + "|NA|" + strPayAmt + "|NA|NA|NA|" + strCurrencyType + "|NA|R|" + strSecurityID + "|NA|NA|F|" + strName + "|" + applicationId + "|Registration|NA|NA|NA|NA|" + strRU + "";
                        String data = strMerchantCode + "|" + strSrno + "|NA|" + strPayAmt + "|NA|NA|NA|" + strCurrencyType + "|NA|R|" + strSecurityID + "|NA|NA|F|" + applicationId + "|" + applicationId + "|MSKVYCommittment|NA|NA|NA|NA|" + strRU + "";
