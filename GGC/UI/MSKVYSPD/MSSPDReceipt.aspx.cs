...
{
    public partial class MSSPDReceipt : System.Web.UI.Page
    {
        protected static readonly ILog log = LogManager.GetLogger(typeof(MSSPDReceipt));
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                ...
                //TestCase
                //string[] strParams = $"4MSETCLRC|340|YHMP2233292761|1|NA|NA|NA|INR|NA|R|msetclrc|NA|NA|{DateTime.Today:yyyy-MM-dd}|0300|F|8820700087|8820700087|MSKVYRegistration|NA|NA|NA|NA|https://gctest.mahatransco.in:9999/UI/MSKVY/MSKVYPayRegReceipt.aspx||".Split('|');
                        ...
                        try
                        {
                            ...
                            switch (mySqlConnection.State)
                            {
                                ...
                                case System.Data.ConnectionState.Open:
                                    ...
                                    strQuery = "update MSKVY_applicantdetails_SPD  set WF_STATUS_CD_C=5,APP_STATUS_DT=CURDATE() ,app_status='REGISTRATION PAYMENT DONE.PAYMENT APPROVAL PENDING.' ,  CustomerID='" + strParams[1] + "' where Application_No='" + strParams[17] + "'";
                                    ...
                                    //strQuery = "select distinct a.*  from MSKVY_applicantdetails_spd  a where a.application_no='" + strParams[17] + "'";
                                    strQuery = "SELECT mas.MEDAProjectID" +
                                        ...
                                        " FROM mskvy_applicantdetails_spd mas" +
                                        $" WHERE mas.APPLICATION_NO='{strParams[17]}'";
                                    ...
                                    var emialIds = new List<string>();

                                    MySqlCommand cmdMob = new MySqlCommand($"SELECT e.SRNO, e.SAPID, e.EMP_NAME, e.EmpEmailID, e.EmpMobile, e.ROLE_ID FROM empmaster e WHERE e.ROLE_ID IN ({RoleConst.STU},{RoleConst.FD})", mySqlConnection);
                                    DataSet dsMob = new DataSet();
                                    MySqlDataAdapter daMob = new MySqlDataAdapter(cmdMob);
                                    daMob.Fill(dsMob);
                                    
                                    mobileNumbers.AddRange(dsMob.Tables[0].AsEnumerable().Where(a => !string.IsNullOrEmpty(a["EmpMobile"].ToString())).Select(a => a["EmpMobile"].ToString().Trim()).Distinct());

                                    emialIds.AddRange(dsMob.Tables[0].AsEnumerable().Where(a => !string.IsNullOrEmpty(a["EmpEmailID"].ToString())).Select(a => a["EmpEmailID"].ToString().Trim()).Distinct());
                                    ...
                                    if (dsAppDet.Tables[0].Rows.Count > 0)
                                    {
                                        ...
                                        strBody += "Developer / Promoter Name : " + dsAppDet.Tables[0].Rows[0]["SPV_Name"].ToString() + "<br/>";
                                        ...
                                        try
                                        {
                                            ...
                                            Msg.Subject = "Online Grid connectivity application registration fees payment.";
                                            Msg.Body = strBody;
                                        ...
                                        SMS.Send(message: SMSTemplates.ProcessingFeePaidForGC(applicationNo: strParams[17], spvName: dsAppDet.Tables[0].Rows[0]["SPV_Name"].ToString()), mobileNumbers.JoinStrings(), MethodBase.GetCurrentMethod(), log);
                                        ...
                                        //    strQueryMob = "select * from empmaster where role_id in (53,31)";
                                        ...
                                        //        //string strURL = ConfigurationManager.AppSettings["SMSURL"].ToString() + "?SenderId=" + ConfigurationManager.AppSettings["SMSSENDER"].ToString() + "&Message=Payment%20of%20processing%20fee%20towards%20Grid%20Connectivity%20application%20no." + strParams[17] + "%20is%20received.%5CnRegards%2C%20C.E.%20STU%2C%20MSETCL&MobileNumbers=" + strMobNos.Remove(strMobNos.Length - 1, 1) + "&ApiKey=" + ConfigurationManager.AppSettings["SMSAPIKEY"].ToString() + "&ClientId=" + ConfigurationManager.AppSettings["SMSCLIENTID"].ToString();
                                        ...
                                        //        SMS.Send(message: SMSTemplates.PaymentProcessingFeeReceivedForGC(applicationNo: strParams[17], spvName: dsAppDet.Tables[0].Rows[0]["PROMOTOR_NAME"].ToString()), strMobNos.Remove(strMobNos.Length - 1, 1), MethodBase.GetCurrentMethod(), log);
                                        //    }
                                        //    catch (Exception ex)
                                        //    {
                                        //        string ErrorMessage = "Method Name: " + MethodBase.GetCurrentMethod().Name + " | Description: " + ex.Message + " " + ex.InnerException;
                ...
                switch (mySqlConnection.State)
                {
                    ...
                    case System.Data.ConnectionState.Open:
                        ...
                        strQuery = "SELECT mas.MEDAProjectID" +
                            ...
                            ", mas.SPV_Name" +
                            ", mas.sub_project_code" +
                            ", ma.GENERATION_VOLTAGE" +
                            ", ma.POINT_OF_INJECTION" +
                            ", ma.INJECTION_VOLTAGE" +
                            ", ma.TYPE_OF_GENERATION" +
                            " FROM mskvy_applicantdetails_spd mas" +
                            " LEFT JOIN mskvy_applicantdetails ma ON ma.APPLICATION_NO = mas.APPLICATION_NO" +
                            " WHERE mas.APPLICATION_NO = '8820700087'";
                        ...
                        objGetStatusDTO.GENERATOR_NAME = dsResult.Tables[0].Rows[0]["SPV_Name"].ToString();
                        ...
                        objGetStatusDTO.PAYMENT_TYPE = "MSKVYSPDRegistration";
                        objGetStatusDTO.Txn_Date = txnDt;
                        objGetStatusDTO.statusId = "5";
                        ...
                        strQuery = "SELECT TxnNo," +
                            "TxnAmount," +
                            "TxnDate " +
                            " FROM billdesk_txn " +
                            $" WHERE MEDAProjectID = '{projId}'" +
                            " and AuthStatus = '0300' " +
                            " and typeofpay = 'MSKVYRegistration'";
