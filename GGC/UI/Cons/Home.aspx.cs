...
//                            Response.Redirect("http://regrid.mahadiscom.in/reGrid/getUsrDtls?usrName='" + strUserID + "'&entity=MSETCL&secKey="+str, false);
        ...
        protected void btnLogin_Click(object sender, EventArgs e)
        {
            ...
            string strGSTIN, strPass;
            strGSTIN = txtGSTIN.Text;
            ...
            try
            {
                ...
                switch (mySqlConnection.State)
                {
                    ...
                    case System.Data.ConnectionState.Open:
                        string strQuery = string.Empty;
                        MySqlCommand cmd  = new MySqlCommand("select *  from APPLICANT_LOGIN_MASTER where USER_NAME='" + strGSTIN + "' and PASSWORD='" + strPass + "' and isActive='Y' ", mySqlConnection);
                        DataSet dsResult = new DataSet();
                        MySqlDataAdapter da = new MySqlDataAdapter(cmd);
                        da.Fill(dsResult);
                        log.Error("dsResult.Tables[0].Rows.Count " + dsResult.Tables[0].Rows.Count.ToString());
                        if (dsResult.Tables[0].Rows.Count > 0)
                        {
                            cmd = new MySqlCommand("select *  from APPLICANTDETAILS where USER_NAME='" + strGSTIN + "'", mySqlConnection);
                            DataSet dsResult2 = new DataSet();
                            MySqlDataAdapter da2 = new MySqlDataAdapter(cmd);
                            da2.Fill(dsResult2);
                            if (dsResult2.Tables[0].Rows.Count > 0)
                            {
                                Session["isValid"]="true";
                                Session["GSTIN"] = dsResult.Tables[0].Rows[0]["GSTIN_NO"].ToString();
                                Session["user_name"] = txtGSTIN.Text;
                        ...
                        {
                            //lblResult.Text = "Login unsuccessfull.";
                            //string scriptText = "alert('Login unsuccessfull. '); window.location='" + "ConsumerDetail.aspx';";
