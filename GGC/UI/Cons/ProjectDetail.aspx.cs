...
namespace GGC.UI.Cons
{
    public partial class ProjectDetail : System.Web.UI.Page
    {
        ...
        {
            string strNAME_TRANS_LICENSEE,  strINJECTION_VOLTAGE, strPOINT_OF_INJECTION, strDISTANCE_FROM_PLANT;
            string strGENERATION_VOLTAGE = "0.0", strLAND_IN_POSSESSION = "", strTOTAL_PRIVATE_LAND = "";
            string strVOLT_LEVEL_SUBSTATION, strSALE_OF_POWER, strINTER_STATE, strTOTAL_REQUIRED_LAND; 
            string  strTOTAL_FOREST_LAND, strSTATUS_FOREST_LAND, strBIRD_SANCTURY_ETC;
            ...
            //strGENERATION_VOLTAGE = txtGenVolt.Text;
            strINJECTION_VOLTAGE = txtInjVolt.Text;
            ...
            try
            {
                ...
                switch (mySqlConnection.State)
                {
                    ...
                    case System.Data.ConnectionState.Open:
                        ...
                        Response.Write("<script language='javascript'>window.alert('" + scriptText + "');window.location='ProjectDetail.aspx';</script>");
                        //if (Session["apptype"].ToString().Contains("Consumer") || Session["apptype"].ToString()=="Conventional Generator")


                        if (Session["isAppliedMEDAID"].ToString() == "Y")
                        {
                            Response.Redirect("~/UI/Cons/UploadGCLetter.aspx", false);

                        }
                        else
                        {
                            Response.Redirect("~/UI/Cons/UploadCon.aspx", false);
                        }
            ...
            try
            {
                ...
                switch (mySqlConnection.State)
                {
                    ...
                    case System.Data.ConnectionState.Open:
                        string strQuery = string.Empty;
                        MySqlCommand cmd;
                        cmd = new MySqlCommand("select * from APPLICANTDETAILS where APPLICATION_NO='" + appId + "'", mySqlConnection);
                        DataSet dsResult = new DataSet();
                        MySqlDataAdapter da = new MySqlDataAdapter(cmd);
                        da.Fill(dsResult);
